...
import {
  ...
  FileText, User, Send, CheckCircle2, Circle, Upload,
} from "lucide-react";
...
function ScoreBadge({ score }: { score: number }) {
  ...
  const color = pct >= 70 ? "#10B981" : pct >= 50 ? "#F59E0B" : "#94A3B8";
  return (
    <span
...
function RoadmapCard({ hasSkills }: { hasSkills: boolean }) {
  ...
  const load = async (force = false) => {
    ...
    } catch { /* no skills or server error */ }
    finally { setLoading(false); }
  };

  useEffect(() => { if (hasSkills) load(); }, [hasSkills]);
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <p className="text-sm text-muted-foreground">Génération de votre roadmap…</p>
            </motion.div>
          ) : !hasSkills ? (
...
function ProfileCompletenessCard({
  ...
  user: { first_name?: string; last_name?: string; skills?: string[]; domain?: string } | null;
  hasCv: boolean;
}) {
  const steps = [
    { label: "CV uploadé",            done: hasCv,                                             href: "/cv"      },
    { label: "Compétences détectées", done: (user?.skills?.length ?? 0) > 0,                   href: "/profile" },
    { label: "Profil complété",       done: !!(user?.first_name && user?.last_name && user?.domain), href: "/profile" },
    { label: "Premier matching lancé", done: false,                                             href: "#generate" },
  ];
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">Complétez votre profil</h2>
        </div>
      ...
      <div className="space-y-2.5">
        {steps.map(({ label, done, href }) => (
          <Link key={label} href={href} className={`flex items-center gap-2.5 group ${done ? "pointer-events-none" : ""}`}>
            <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 transition-colors ${
              done ? "bg-emerald-500/20" : "bg-muted group-hover:bg-primary/10"
            }`}>
              {done
            ...
            <span className={`text-xs ${done ? "line-through text-muted-foreground/50" : "text-muted-foreground group-hover:text-foreground transition-colors"}`}>
              {label}
            </span>
          </Link>
        ))}
...
function QuickActions() {
  const actions = [
    { label: "Parcourir les offres", href: "/jobs",         icon: Briefcase, gradient: "gradient-bg" },
    { label: "Mon CV",              href: "/cv",           icon: FileText,  gradient: "gradient-bg-emerald" },
    { label: "Mon profil",          href: "/profile",      icon: User,      gradient: "gradient-bg-amber" },
    { label: "Mes candidatures",    href: "/applications", icon: Send,      gradient: "gradient-bg" },
  ];
  return (
    <motion.div
    ...
    >
      {actions.map(({ label, href, icon: Icon, gradient }) => (
        <Link key={label} href={href}>
          <div className="card-base card-hover p-4 flex flex-col gap-3 h-full cursor-pointer group">
            <div className={`w-9 h-9 rounded-xl ${gradient} flex items-center justify-center`}>
              <Icon className="w-4 h-4 text-white" />
            </div>
            <p className="text-xs font-semibold text-foreground group-hover:text-primary transition-colors leading-snug">{label}</p>
          </div>
        </Link>
      ))}
    </motion.div>
...
export default function DashboardPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const msg =
        ...
        "Uploadez un CV d'abord pour obtenir des correspondances.";
      setGenError(msg);
    } finally {
      setGenerating(false);
    }
  ...
  const stats = [
    { icon: Target,     label: "AI Matches",        value: recs.length,                        gradient: "gradient-bg",         delay: 0    },
    ...
    { icon: Star,       label: "Mes compétences",    value: user?.skills?.length ?? 0,          gradient: "gradient-bg",         delay: 0.18 },
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {genError && (
            <p className="text-[11px] text-destructive text-right max-w-xs">{genError}</p>
          )}
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
        ...
        {/* Right column: domain jobs + completeness */}
        <div className="xl:col-span-2 space-y-5">
          ...
          >
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold">Recommandé pour vous</h2>
              </div>
              <Link href="/jobs">
                <Button variant="ghost" size="sm" className="h-7 text-xs px-2 gap-1 text-muted-foreground hover:text-foreground">
            ...
            <div className="divide-y divide-border">
              {loading ? (
                ...
                <p className="text-sm text-muted-foreground p-6 text-center">
                  Aucune offre pour votre domaine.
                </p>
              ) : domainJobs.map((job) => {
          ...
          <ProfileCompletenessCard user={user} hasCv={hasCv === true} />
        </div>
      </div>
