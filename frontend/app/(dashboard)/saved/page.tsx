...
import { BookmarkCheck, MapPin, Building2, ExternalLink, Bookmark, Clock } from "lucide-react";

function SavedJobCard({ job, index }: { job: Job; index: number }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      {loading ? (
        ...
        >
          {jobs.map((job, i) => <SavedJobCard key={job.id} job={job} index={i} />)}
        </motion.div>
      )}
    </div>
  );
